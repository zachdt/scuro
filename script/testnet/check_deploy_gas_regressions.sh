...
ROOT="$(repo_root)"
THRESHOLDS_PATH="${ROOT}/script/testnet/deploy-gas-thresholds.json"
...
forge build \
  ...
  script/testnet/DeployFinalize.s.sol >/dev/null
...
echo "[gas-check] running staged testnet deploy"
PRIVATE_KEY="${ADMIN_KEY}" bash "${ROOT}/script/testnet/deploy_staged.sh" "${RPC_URL}" >/dev/null
...
if total_gas > thresholds["staged_deploy"]["full_testnet_deploy_total"]:
    failures.append(
        f"full testnet deploy used {total_gas} gas (max {thresholds['staged_deploy']['full_testnet_deploy_total']})"
    )
