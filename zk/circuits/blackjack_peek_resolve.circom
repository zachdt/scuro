...
template BlackjackPeekResolve() {
    ...
    for (var playerHashIndex = 0; playerHashIndex < 38; playerHashIndex++) {
        playerHash.inputs[playerHashIndex] <== playerHashInputs[playerHashIndex];
    }
    ...
    for (var cipherIndex = 0; cipherIndex < 12; cipherIndex++) {
        playerCipherInputs[cipherIndex + 2] <== privatePlayerCards[cipherIndex];
        dealerCipherInputs[cipherIndex + 2] <== privateDealerCards[cipherIndex];
    }
    playerCipherInputs[14] <== playerCipherSalt;
    dealerCipherInputs[14] <== dealerCipherSalt;
    for (var inputIndex = 0; inputIndex < 15; inputIndex++) {
        playerCipherHash.inputs[inputIndex] <== playerCipherInputs[inputIndex];
        dealerCipherHash.inputs[inputIndex] <== dealerCipherInputs[inputIndex];
    }
