...
contract SmokeRealProofHands is Script {
    ...
    function run() external {
        ...
        bool forcePrecompileOnly = vm.envOr("FORCE_PRECOMPILE_ONLY", false);

        _assertVerifierMetadata(
            catalog, address(pokerEngine), pokerVerifierBundle, address(blackjackEngine), blackjackVerifierBundle
        );

        if (forcePrecompileOnly) {
            vm.startBroadcast(adminKey);
            PokerVerifierBundle(pokerVerifierBundle).setVerifiers(address(0), address(0), address(0));
            BlackjackVerifierBundle(blackjackVerifierBundle).setVerifiers(address(0), address(0), address(0));
            vm.stopBroadcast();
        }
